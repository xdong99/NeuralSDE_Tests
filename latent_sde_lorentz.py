...
def vis(xs, ts, latent_sde, bm_vis, img_path, num_samples=10):
    ...
    colormap = cm.viridis  # You can change 'viridis' to any other colormap like 'jet', 'plasma', etc.
    color_step = int(255 / num_samples)

    for i in range(num_samples):
        row = i + 1
        color = colormap(i * color_step)

        ax = plt.subplot(gs[row, 0], projection='3d')
        sample_xs = xs[:, i, :].cpu().numpy()
        z1, z2, z3 = np.split(sample_xs, indices_or_sections=3, axis=-1)
        ax.plot(z1[:, 0], z2[:, 0], z3[:, 0], color=color, label = f"Sample {i+1}")
        ax.scatter(z1[0, 0], z2[0, 0], z3[0, 0], color=color, marker='x')
        ax.set_yticklabels([])
        ax.set_xticklabels([])
        ax.set_zticklabels([])
        ...
        ax2 = plt.subplot(gs[row, 1], projection='3d')
        z1, z2, z3 = np.split(generated_xs, indices_or_sections=3, axis=-1)
        ax2.plot(z1[:, i, 0], z2[:, i, 0], z3[:, i, 0], color=color, label = f"Sample {i+1}")
        ax2.scatter(z1[0, i, 0], z2[0, i, 0], z3[0, i, 0], color=color, marker='x')
        ax2.set_yticklabels([])
        ax2.set_xticklabels([])
        ax2.set_zticklabels([])
        ax2.set_xlabel('$z_1$', labelpad=0., fontsize=16)
        ax2.set_ylabel('$z_2$', labelpad=.5, fontsize=16)
        ax2.set_zlabel('$z_3$', labelpad=0., horizontalalignment='center', fontsize=16)
        ax2.set_title(f'Generated Trajectory {i+1}', fontsize=20)
        ax2.set_xlim(xlim)
        ax2.set_ylim(ylim)
        ax2.set_zlim(zlim)
